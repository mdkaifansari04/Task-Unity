...
exports.isAdmin =async (req,res,next) =>{
    try {
            ...
            if(!adminExits) return res.status(404).json({success : false, message : "Admin does not exist"})
            next()
    } catch (error) {
        res.status(500).json({ success: false, message: 'Authentication failed ', error: error.message });
    }
...
exports.isUser =async (req,res,next) =>{
    try {
            ...
            if(!userExits) return res.status(404).json({success : false, message : "User does not exist"})
            next()
    } catch (error) {
        res.status(500).json({ success: false, message: 'Authentication failed ', error: error.message });
    }
